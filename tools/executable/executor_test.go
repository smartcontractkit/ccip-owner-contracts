...
func setupSimulatedBackendWithMCMS() (*ecdsa.PrivateKey, *bind.TransactOpts, *simulated.Backend, *configwrappers.WrappedManyChainMultisig, error) {
	...
	_, tx, mcms, err := configwrappers.DeployWrappedManyChainMultisig(auth, sim.Client())
	if err != nil {
		return nil, nil, nil, nil, err
	}
	...
	receipt, err := bind.WaitMined(auth.Context, sim.Client(), tx)
	if err != nil {
		return nil, nil, nil, nil, err
	}
	...
	// Set a valid config
	tx, err = mcms.SetConfig(auth, &configwrappers.Config{
		Quorum: 1,
...
func TestExecutor_ExecuteE2E_SingleChainSingleSignerSingleTX_Success(t *testing.T) {
	...
	tx, err = executor.SetRootOnChain(auth, "1337")
	assert.NoError(t, err)
	assert.NotNil(t, tx)
	sim.Commit()
	...
	tx, err = executor.ExecuteOnChain(auth, 0)
	assert.NoError(t, err)
	assert.NotNil(t, tx)
	sim.Commit()
...
func TestExecutor_ExecuteE2E_SingleChainSingleSignerMultipleTX_Success(t *testing.T) {
	...
	for i := 0; i < 4; i++ {
		// Execute the proposal
		tx, err = executor.ExecuteOnChain(auth, i)
		assert.NoError(t, err)
		assert.NotNil(t, tx)
		sim.Commit()
