...
var (
	abigenVersion = getenvWithDefault("ABIGEN_VERSION", "1.13.8")
	goBinaryPath = getenvWithDefault("GO_BINPATH", "go")
)
...
// main is the entry point for the abigen wrapper. It takes 4 positional
// arguments: abiPath binPath, className and pkgName
func main() {
	abiPath := os.Args[1]
	binPath := os.Args[2]
	className := os.Args[3]
...
func Abigen(a AbigenArgs) {
	...
	buildCommand := exec.Command(goBinaryPath, args...)
